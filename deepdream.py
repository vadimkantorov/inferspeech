...
if __name__ == '__main__':
	...
	signal = torch.from_numpy(signal).to(torch.float32).requires_grad_()
	labels = torch.IntTensor([{' ' : 0, "'" : 27}.get(c, 1 + ord(c) - ord('A')) for c in args.transcript])
	...
	for i in range(args.num_iter):
		...
		loss = torch.nn.functional.ctc_loss(F.log_softmax(scores, dim = 1).permute(2, 0, 1), labels.unsqueeze(0), torch.IntTensor([scores.shape[-1]]), torch.IntTensor([len(labels)]))
		model.zero_grad()
		loss.backward()
		signal.grad.data[torch.isnan(signal.grad.data)] = 0
		signal.data.sub_(signal.grad.data.mul_(args.lr))
		signal.grad.data.zero_()
		print(i, float(loss))

	scipy.io.wavfile.write(args.output_path, sample_rate, signal.to(torch.int16).numpy())
